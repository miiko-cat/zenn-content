...
サーバ上で定義したSupabaseのDB定義を変更したいが、サーバ上で変更すると、すぐ本番に反映されてしまいます。
それを避けるため、サーバ上のDB定義をローカルに反映し、定義変更とデプロイする方法を説明します。

#### ローカルDBに定義を反映する方法
次のコマンドでサーバ上のDB定義を、ローカルにマイグレーションファイルファイルとして、作成することができます。
※Supabaseへのログイン、プロジェクトの紐づけ、DockerDesktopの起動済みが前提です。
```
supabase db pull
```
このコマンドを実行することで、`supabase/migrations`にマイグレーションファイルが自動的に作成されます。

その後、次のコマンドでDB定義をローカルに反映できます。
```
supabase db reset
```

#### サーバー上のデータを取得し、ローカルに反映する方法
サーバ上のデータを取得し、ローカルのDBにもデータを反映したい場合、次のコマンドでSQLファイルを作成します。
```
supabase db dump -f supabase/seed.sql --data-only
```
`--data-only`フラグを指定することで、データのみを取得します。

その後、次のコマンドでDB定義をローカルに反映できます。
```
supabase db reset
```
DB定義を反映するときと同じですが、まずは`supabase/migrations`に格納されたマイグレーションファイルが実行され、DB定義が作成されます。
その後、`supabase/seed.sql`がシードデータ(テストデータ)として、挿入されることになります。
